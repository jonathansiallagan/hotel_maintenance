<x-admin-layout>
                ...
                @forelse($schedules as $schedule)
                    <tr>
                        ...
                        <td class="px-6 py-4 text-right">
                            <form action="{{ route('admin.maintenance.destroy', $schedule->id) }}" method="POST"
            ...
            <form action="{{ route('admin.maintenance.store') }}" method="POST" class="space-y-4">
                ...
                <div class="flex justify-end gap-2 pt-4 border-t mt-4">
                    <button type="button" onclick="document.getElementById('createModal').close()"
                        class="px-4 py-2 text-gray-500 font-bold hover:bg-gray-100 rounded-lg">Batal</button>
                    <button type="submit"
                        class="bg-[#D0BBB8] hover:bg-[#bda3a0] text-white px-4 py-2 rounded-lg font-bold shadow-sm">Simpan
                        Jadwal</button>
                </div>
            </form>
        </div>
    ...
    {{-- SCRIPT UNTUK MEMBUKA MODAL OTOMATIS DARI DASHBOARD RCA --}}

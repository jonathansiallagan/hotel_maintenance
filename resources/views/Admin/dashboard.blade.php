<x-admin-layout>
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-sm text-left">
                    ...
                    <tbody class="divide-y divide-gray-100">
                        ...
                        @empty
                        <tr>
                            <td colspan="4" class="px-6 py-12 text-center text-gray-400">Belum ada tiket terbaru.</td>
                        </tr>
                        @endforelse
                    </tbody>
                </table>
                ...
                <div class="space-y-4">
                    <div onclick="window.location.href='{{ route('admin.assets.index') }}'" class="flex items-center justify-between p-4 bg-blue-50/50 rounded-xl border border-blue-100 hover:bg-blue-50 transition cursor-pointer group">
                        <div class="flex items-center gap-4">
                            <div class="w-12 h-12 bg-blue-100 rounded-lg flex items-center justify-center text-blue-600 shadow-sm group-hover:scale-110 transition-transform"><i class="fa-solid fa-boxes-stacked text-xl"></i></div>
                            <div>
                    ...
                    <div onclick="window.location.href='{{ route('admin.users.index') }}'" class="flex items-center justify-between p-4 bg-purple-50/50 rounded-xl border border-purple-100 hover:bg-purple-50 transition cursor-pointer group">
                        <div class="flex items-center gap-4">
                            <div class="w-12 h-12 bg-purple-100 rounded-lg flex items-center justify-center text-purple-600 shadow-sm group-hover:scale-110 transition-transform"><i class="fa-solid fa-users text-xl"></i></div>
                            <div>
    ...
    <div class="bg-[#0A2647] rounded-3xl p-8 shadow-xl text-white mb-8">
        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8 pb-6 border-b border-blue-800/50">
            <div>
            ...
            <form action="{{ url()->current() }}" method="GET" class="flex items-center gap-3 bg-white/5 p-2 rounded-xl backdrop-blur-sm border border-white/10">
                <select name="month" class="bg-transparent border-none text-sm text-white focus:ring-0 cursor-pointer [&>option]:text-gray-900">
                ...
                <select name="location_id" class="bg-transparent border-none text-sm text-white focus:ring-0 cursor-pointer [&>option]:text-gray-900">
                ...
                <button type="submit" class="bg-blue-600 hover:bg-blue-500 text-white px-4 py-2 rounded-lg text-xs font-bold transition">
                    Filter
                </button>
            </form>
        </div>
        ...
        <div class="bg-white rounded-2xl p-6 mb-8 shadow-inner">
            <h3 class="text-gray-800 font-bold mb-4 text-sm uppercase tracking-wider">Komposisi Masalah Seluruh Aset</h3>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div class="bg-white/10 border border-white/10 rounded-2xl p-6 backdrop-blur-md">
                <div class="flex justify-between items-start mb-4">
                    <div>
                        <div class="text-red-400 text-[10px] font-bold uppercase tracking-widest mb-1">Peringkat {{ $loop->iteration }} Rusak</div>
                        <h4 class="text-lg font-bold">{{ $categoryName }}</h4>
                    </div>
                    <div class="w-8 h-8 rounded-full bg-white/10 flex items-center justify-center font-bold text-xs">
                        {{ array_sum($causes) }}x
                    </div>
                </div>

<nav x-data="{ open: false }" class="bg-white border-b border-gray-100">
    ...
    {{-- 1. LOGIC PENENTU ROUTE (SUDAH BENAR) --}}
    @php
    ...
    // Tentukan Active State (Untuk highlight menu)
    $isDashboardActive = request()->routeIs('admin.dashboard') ||
    request()->routeIs('technician.dashboard') ||
        ...
        <div class="flex justify-between h-16">
            <div class="flex">
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                    ...
                    {{-- Menu Admin --}}
                    @if($userRole === 'admin')
                    <x-nav-link :href="route('profile.edit')" :active="request()->routeIs('admin.users.*')">
                        {{ __('Manage Users') }}
                    </x-nav-link>
                    @endif
                </div>
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            {{-- PERBAIKAN: Gunakan variable $dashboardRoute dan $isDashboardActive --}}
